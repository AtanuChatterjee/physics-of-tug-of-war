...
def simulate_pair(pair, iterations, F0_left, F0_right):
    wK_left, wK_right = pair
    left_wins = 0
    right_wins = 0
    ties = 0
    for _ in range(iterations):
        outcome = run_simulation_outcome(
            omega_left=wK_left,       # parameter sweep over this
            omega_right=wK_right,     # parameter sweep over this
            noise_std=0.02,            # fixed noise
            K_left=1.0,               # fixed K for left and right teams
            K_right=1.0,
            F0_left=F0_left,          # set same or different pulling strengths
            F0_right=F0_right
        )
        if outcome == "left team wins":
            ...
            ties += 1
    return pair, left_wins, right_wins, ties

def parameter_sweep(wK_values, iterations, F0_left=5.0, F0_right=5.0):
    num_values = len(wK_values)
    left_wins_matrix = np.zeros((num_values, num_values), dtype=int)
    right_wins_matrix = np.zeros((num_values, num_values), dtype=int)
    ties_matrix = np.zeros((num_values, num_values), dtype=int)
    parameter_pairs = list(itertools.product(wK_values, wK_values))

    tasks = [(pair, iterations, F0_left, F0_right) for pair in parameter_pairs]

    num_workers = None

    with concurrent.futures.ProcessPoolExecutor(max_workers=num_workers) as executor:
        futures = {executor.submit(simulate_pair, pair, iter_count, f0l, f0r): pair
                   for (pair, iter_count, f0l, f0r) in tasks}

        for future in concurrent.futures.as_completed(futures):
            pair, left_wins, right_wins, ties = future.result()
            wK_left, wK_right = pair
            i = np.where(wK_values == wK_right)[0][0]  # Row index
            j = np.where(wK_values == wK_left)[0][0]   # Column index
            left_wins_matrix[i, j] = left_wins
            right_wins_matrix[i, j] = right_wins
            ties_matrix[i, j] = ties
            print(f"Left wK: {wK_left:.2f}, Right wK: {wK_right:.2f}, "
                  f"Left Wins: {left_wins}, Right Wins: {right_wins}, Ties: {ties}")

    np.savez('tug_of_war_contests_n_0_02.npz',
             left_wins=left_wins_matrix,
             right_wins=right_wins_matrix,
             ties=ties_matrix,
             wK_values=wK_values,
             iterations=iterations,
             F0_left=F0_left,
             F0_right=F0_right)
    return left_wins_matrix, right_wins_matrix, ties_matrix

if __name__ == "__main__":
    print("Precompiling Numba functions...")
    # Precompile the functions for faster execution
    run_simulation_outcome(0.0, 0.0, 0.0, 1.0, 1.0, F0_left=5.0, F0_right=5.0)
    print("Precompilation completed.")

    wK_values = np.linspace(0, 1, 50)

    # Perform the parameter sweep with parallel processing
    print("Starting parameter sweep...")
    results = parameter_sweep(wK_values, iterations=50, F0_left=5.0, F0_right=5.0)
